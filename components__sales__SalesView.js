...
import { downloadProfessionalTablePDF, downloadProfessionalTableImage, downloadProfessionalTableExcel, downloadProfessionalInvoicePDF, downloadProfessionalInvoiceImage, downloadProfessionalInvoiceExcel } from './utils__professionalExport.js?v=7.9.4.44-purchase-shipping';
import { getBrandLogoDataUrl, DEFAULT_LOGO_DATA_URL } from './brand__logo.js?v=7.9.4.44-purchase-shipping';
import { SalesInvoiceEditModal } from './components__sales__InvoiceEditModal.js?v=7.9.4.44-purchase-shipping';
...
export const SalesView = () => {
    ...
    const getSalesExportData = () => {
        const headers = ['رقم الفاتورة', 'النوع', 'التاريخ', 'العميل', 'الصنف', 'الوحدة', 'الكمية', 'السعر', 'إجمالي الصنف', 'الشحن', 'تحمل الشحن', 'إجمالي الفاتورة', 'المدفوع', 'المتبقي'];
        const rows = [];
        filteredInvoices.forEach((inv) => {
            ...
            items.forEach((item) => rows.push([
                ...
                Number(item?.total || 0),
                Number(inv.shippingCost || 0),
                Number(inv.shippingCost || 0) > 0 ? (inv.shippingChargeMode === 'profit' ? 'من أرباح الفاتورة' : 'على العميل') : '-',
    ...
    const getInvoiceCustomer = (inv) => customers.find((c) => c.id === inv?.customerId) || null;
    const normalizeEgyptWhatsAppPhone = (phone) => {
    ...
    const buildInvoiceShareText = (inv) => {
        ...
        const lines = [
            ...
            '--------------------',
            ...(inv.items || []).map((item) => `${item.productName} | ${item.quantity} ${item.unitName || ''} × ${Number(item.unitPrice || 0).toFixed(2)} = ${Number(item.total || 0).toFixed(2)} ${symbol}`),
            '--------------------',
            `الإجمالي: ${Number(inv.subtotal || 0).toFixed(2)} ${symbol}`,
            Number(inv.discountTotal || 0) ? `الخصم: ${Number(inv.discountTotal || 0).toFixed(2)} ${symbol}` : '',
            Number(inv.taxTotal || 0) ? `الضريبة: ${Number(inv.taxTotal || 0).toFixed(2)} ${symbol}` : '',
            Number(inv.shippingCustomerAmount || (inv.shippingChargeMode === 'customer' ? inv.shippingCost : 0)) ? `الشحن: ${Number(inv.shippingCustomerAmount || inv.shippingCost).toFixed(2)} ${symbol}` : '',
            `الصافي: ${Number(inv.grandTotal || 0).toFixed(2)} ${symbol}`,
            `المدفوع: ${Number(inv.paidAmount || 0).toFixed(2)} ${symbol}`,
            Number(inv.remainingAmount || 0) ? `المتبقي: ${Number(inv.remainingAmount || 0).toFixed(2)} ${symbol}` : '',
    ...
    const handleSendWhatsApp = (inv) => {
        ...
        const phone = normalizeEgyptWhatsAppPhone(customer?.phone || inv?.customerPhone);
        if (!phone) {
            showToast('لا يوجد رقم هاتف مسجل لهذا العميل. أضف الرقم من ملف العميل أولاً.', 'warning');
            return;
        }
        const text = buildInvoiceShareText(inv);
        window.open(`https://wa.me/${phone}?text=${encodeURIComponent(text)}`, '_blank', 'noopener,noreferrer');
    };
    const handleSendSMS = (inv) => {
        const customer = getInvoiceCustomer(inv);
        const phone = String(customer?.phone || inv?.customerPhone || '').trim();
        if (!phone) {
            showToast('لا يوجد رقم هاتف مسجل لهذا العميل. أضف الرقم من ملف العميل أولاً.', 'warning');
            return;
        }
