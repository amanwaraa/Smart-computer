/* Smart Computer Activation Runtime v1
...
(() => {
  ...
  function activationFileName(payload,fileName){return (fileName||`${payload.companyName||'Smart computer'}-login.mzauth`).replace(/[\\/:*?"<>|]+/g,'_').replace(/\.(?:mzauth|ctauth)$/i,'')+'.mzauth'}
  ...
  function readRuntime(){try{const raw=localStorage.getItem(RUNTIME_KEY),value=raw?JSON.parse(unwrapText(raw)):null;if(value&&Object.prototype.hasOwnProperty.call(value,'ai')){delete value.ai;localStorage.setItem(RUNTIME_KEY,wrapText(JSON.stringify(value)));}return value}catch(_){return null}}
  ...
  function readMasterConfig(){try{const raw=localStorage.getItem(MASTER_KEY),value=raw?JSON.parse(unwrapText(raw)):null;if(value&&Object.prototype.hasOwnProperty.call(value,'ai')){delete value.ai;localStorage.setItem(MASTER_KEY,wrapText(JSON.stringify(value)));}return value}catch(_){return null}}
