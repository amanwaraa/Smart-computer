    ...
    dbOpenPromise = new Promise((resolve, reject) => {
        try {
            ...
            request.onupgradeneeded = (event) => {
                ...
                const obsoleteStores = ['restaurant_tables','restaurant_sections','restaurant_orders','kitchen_sections','table_reservations','recipes','waste_records'];
                obsoleteStores.forEach((storeName) => { if (db.objectStoreNames.contains(storeName)) db.deleteObjectStore(storeName); });
...
export const DEFAULT_SETTINGS = {
    ...
    receiptFooterMessage: 'شكراً لتعاملكم مع Smart computer - نسعد بخدمتكم دائماً',
    barcodePrefix: '21',
...
export async function seedDatabaseDefaults() {
    ...
    if (!settings) {
        ...
        await putInStore('audit_logs', {
            ...
            details: 'تم بدء تشغيل Smart computer بنجاح مع البيانات التأسيسية وشجرة الوحدات.',
        });
    }
}
