...
import { code128Geometry } from './utils__code128.js?v=7.9.4.40-smart-computer-products-syntax-fix';
...
export function documentBrandSettings(extra={}){
  ...
  const storeName=header?.querySelector('h1')?.textContent?.trim() || extra.storeName || 'Smart computer';
...
async function drawBrand(ctx,w,settings,title,subtitle='',top=34){
  ...
  txt(ctx,settings?.storeName||'Smart computer',w/2,y+19,34,900,'center');y+=52;
  if(settings?.subtitle){txt(ctx,settings.subtitle,w/2,y,17,700,'center','#0000fe');y+=30;}
  ...
  line(ctx,32,y,w-32,y,3,'#0000fe');y+=38;
...
function drawTable(ctx,{x,y,w,headers,rows,fontSize=19,headH=60,alternate=true,maxRows=null}){
  const data=maxRows==null?rows:rows.slice(0,maxRows);const cols=tableColumnGeometry(headers,x,w);roundedRect(ctx,x,y,w,headH,7,'#0000fe','#0000b8',1.2);
...
export async function renderTableCanvas({title='تقرير',subtitle='',headers=[],rows=[],settings={},orientation='landscape',pageNote=''}){
  ...
  y=drawTable(ctx,{x:48,y,w:w-96,headers,rows,fontSize:fs});y+=38;txt(ctx,`تم إنشاء هذا التقرير من نظام ${settings.storeName||'Smart computer'}`,w/2,y,14,600,'center','#94a3b8');return canvas;
}
...
export async function renderVoucherCanvas(voucher,settings={},options={}){
  ...
  roundedRect(ctx,margin,y,w-margin*2,100,12,voucher?.type==='receipt'?'#eff6ff':'#fffbeb',voucher?.type==='receipt'?'#bfdbfe':'#fde68a');txt(ctx,`رقم السند: #${voucher?.voucherNumber||'-'}`,w/2,y+32,22,900,'center');txt(ctx,dateText(voucher?.date),w/2,y+70,17,600,'center','#64748b');y+=132;
  ...
  roundedRect(ctx,margin,y,w-margin*2,92,12,'#f8fafc','#cbd5e1');txt(ctx,'المبلغ:',w-margin-18,y+46,22,900,'right','#334155');txt(ctx,`${money(voucher?.amount)} ${settings.currencySymbol||''}`,margin+18,y+46,30,900,'left','#0000b8');y+=118;
  ...
  line(ctx,margin,y,w-margin,y,1.5,'#cbd5e1');y+=54;txt(ctx,'المستلم',w-margin-80,y,16,700,'center','#64748b');txt(ctx,'أمين الصندوق / الكاشير',margin+120,y,16,700,'center','#64748b');y+=70;line(ctx,w-margin-170,y,w-margin+10,y,1,'#94a3b8');line(ctx,margin+30,y,margin+210,y,1,'#94a3b8');y+=45;txt(ctx,voucher?.userName||'المدير',margin+120,y,14,600,'center','#94a3b8');txt(ctx,`نظام ${settings.storeName||'Smart computer'} - سند مالي رسمي`,w/2,y+60,13,600,'center','#94a3b8');const finalH=Math.ceil(y+105);const out=createCanvas(w,finalH);out.ctx.drawImage(canvas,0,0,w,finalH,0,0,w,finalH);return out.canvas;
}
