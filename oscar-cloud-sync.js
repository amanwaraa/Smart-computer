...
async function prepareRemoteValue(store,key,value,remoteRev,{deleted=false,latestMovementMap=null,repairs=null}={}){
  ...
  if(store==='settings'&&key==='store_config'&&value&&typeof value==='object'){
    try{
      ...
      if(local?.activeWarehouseId){return{skip:false,value:{...value,activeWarehouseId:local.activeWarehouseId}};}
    }catch(_){ }
  ...
  // Legacy zero/delete without its own payload timestamp is dangerous. Accept it only when
  // the latest movement explicitly proves that zero is the real resulting balance.
  if(localQty>0&&remoteQty===0&&!remotePayloadTs){
    ...
    // If there is no trustworthy movement proving zero, keep the non-zero local balance.
    // This specifically protects old Smart Computer installations that had stock rows but
    // incomplete legacy movement history.
...
async function applyRows(rows,batch){
  ...
  // Apply movements first so an old incoming zero can be checked against the real final movement balance.
