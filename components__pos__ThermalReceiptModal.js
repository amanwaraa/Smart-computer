...
export const ThermalReceiptModal = () => {
    const { showThermalModal, setShowThermalModal, settings, customers, showToast } = useApp();
    ...
    const customerPhone = String(invoice.customerPhone || customers.find((c) => c.id === invoice.customerId)?.phone || '').trim();
    const invoiceMessage = () => {
        ...
        return [
            ...
            '--------------------',
            ...(invoice.items || []).map((item) => `${item.productName} | ${item.quantity} ${item.unitName || ''} × ${money(item.unitPrice)} = ${money(item.total)} ${symbol}`),
            '--------------------',
    ...
    const sendWhatsApp = () => {
        ...
        else if (/^1\d{9}$/.test(phone)) phone = '20' + phone;
        if (!phone) return showToast('لا يوجد رقم هاتف مسجل لهذا العميل.', 'warning');
        window.open(`https://wa.me/${phone}?text=${encodeURIComponent(invoiceMessage())}`, '_blank', 'noopener,noreferrer');
    };
    const sendSMS = () => {
        if (!customerPhone) return showToast('لا يوجد رقم هاتف مسجل لهذا العميل.', 'warning');
        const sep = /iPhone|iPad|iPod/i.test(navigator.userAgent || '') ? '&' : '?';
        window.location.href = `sms:${customerPhone}${sep}body=${encodeURIComponent(invoiceMessage())}`;
    };
    ...
    return h('div', {
        ...
        h('div', { className: 'no-print flex flex-wrap items-center justify-between p-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 gap-2 select-none' },
            ...
            h('div', { className: 'flex items-center gap-1.5 flex-wrap justify-end' },
                ...
                h('button', { onClick: sendWhatsApp, className: 'flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-blue-200 bg-blue-50 text-blue-700 text-xs font-bold hover:bg-blue-100', title: 'إرسال كامل الفاتورة والملاحظات عبر واتساب' },
                    h(MessageCircle, { className: 'w-3.5 h-3.5' }), h('span', { className: 'hidden sm:inline' }, 'واتساب')
                ),
                h('button', { onClick: sendSMS, className: 'flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-blue-200 bg-white text-blue-700 text-xs font-bold hover:bg-blue-50', title: 'إرسال كامل الفاتورة والملاحظات عبر SMS' },
                    h(MessageSquare, { className: 'w-3.5 h-3.5' }), h('span', { className: 'hidden sm:inline' }, 'SMS')
                ),
                h('button', { id: 'btn-bluetooth-print', onClick: handleBluetoothPrint, disabled: isExporting, className: `flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-white text-xs font-bold shadow-xs transition disabled:opacity-50 ${printerState.connected ? 'bg-blue-600 hover:bg-blue-700' : 'bg-slate-500 hover:bg-slate-600'}`, title: printerState.connected ? `متصل: ${printerState.name || 'طابعة'}` : 'اربط الطابعة من الإعدادات' },
        ...
        h('div', { className: 'receipt-preview-scroll flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-2 sm:p-4 bg-slate-100 dark:bg-slate-950/80 flex items-start justify-center custom-scrollbar overscroll-contain' },
            h('div', {
                ...
                invoice.notes ? h('div', { className: 'receipt-notes' },
                    h('strong', null, 'ملاحظات:'),
                    h('div', null, invoice.notes)
                ) : null,
                h('div', { className: 'receipt-footer' },
                    ...
                    h('div', { className: 'receipt-system-mark' }, 'Smart computer - POS')
                )
            )
        )
    ));
