...
import { GENERAL_PAGE_PERMISSIONS, normalizeEmployeePermissions } from './utils__permissions.js?v=7.9.4.44-purchase-shipping';
import {
    ...
    isModalOpen && h('div', { className: 'oscar-employee-modal-overlay fixed z-[120] bg-black/55 flex justify-center' },
      h('div', { className: 'oscar-employee-modal-panel w-full max-w-2xl bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col' },
        h('div', { className: 'p-4 border-b border-slate-100 flex items-center justify-between shrink-0' },
          h('div', { className: 'flex items-center gap-2' }, h(ShieldCheck, { className: 'w-5 h-5 text-emerald-600' }), h('div', null, h('div', { className: 'font-black text-slate-900 text-sm' }, editingEmployee ? 'تعديل الموظف والصلاحيات' : 'إضافة موظف جديد'), h('div', { className: 'text-[10px] text-slate-500' }, 'حدد الصفحات وصلاحيات العمليات التي يحتاجها الموظف'))),
